(function (factory) {
    ...
    _require.modules = [
        ...
        function (module, exports) {
            ...
            var defaultClass = 'biz-transfer', dataKey = 'bizTransfer';
            Transfer.prototype = {
                init: function (options) {
                    var tpl = $('\t\t\t<div class="biz-transfer-list">\t\t\t\t<div class="biz-transfer-list-header">\t\t\t\t\t<input type="checkbox" title="' + options.titles[0] + '">\t\t\t\t</div>\t\t\t\t<div class="biz-transfer-list-body"><ul class="biz-transfer-list-content"></ul></div>\t\t\t</div>\t\t\t<div class="biz-transfer-operation">\t\t\t\t<button class="biz-transfer-move-right"><i class="biz-icon">&#xe315;</i></button>\t\t\t\t<button class="biz-transfer-move-left"><i class="biz-icon">&#xe314;</i></button>\t\t\t</div>\t\t\t<div class="biz-transfer-list">\t\t\t\t<div class="biz-transfer-list-header">\t\t\t\t\t<input type="checkbox" title="' + options.titles[1] + '">\t\t\t\t</div>\t\t\t\t<div class="biz-transfer-list-body"><ul class="biz-transfer-list-content"></ul></div>\t\t\t</div>\t\t');
                ...
                addItems: function (data) {
                    var leftData = this.formatInput(data), rightData = this.getTargets(leftData), me = this;
                    ...
                    this.$main.find(':checkbox').bizCheckbox({ theme: this.options.theme });
                },
                ...
                bindSelect: function () {
                    this.$main.on('click.bizTransferSelectAll', '.biz-transfer-list-header .biz-label', function (e) {
                        var selected = $(this).hasClass('biz-checkbox-checked'), list = $(this).parent().next().find(':checkbox').filter(':not(:disabled)');
                        if (selected) {
                        ...
                        var list = $(this).parent().parent(), selectedCount = list.find('.biz-checkbox-checked').length, checkboxCount = list.find(':checkbox').filter(':not(:disabled)').length, selectAll = list.parent().prev().find(':checkbox');
                ...
                createButton: function () {
                    this.$main.find('button').bizButton({ theme: this.options.theme });
                    var me = this;
                ...
                moveRight: function (selected, e) {
                    ...
                    this.$rightListBody.find(':checkbox').bizCheckbox({ theme: this.options.theme });
                ...
                value: function () {
                    var selected = this.$rightListBody.find(':checkbox'), keyMap = this.options.keyMap;
                ...
                destroy: function () {
                    this.$main.removeClass(defaultClass + ' ' + this.options.customClass).find(':checkbox').bizCheckbox('destroy').find('button').bizButton('destroy');
                    this.$main.off('click.bizTransferSelectAll').off('click.bizTransferSelectOne').off('click.bizTransferMoveRight').off('click.bizTransferMoveLeft');
                    this.$main.empty();
                    this.$main.data(dataKey, null);
                }
            };
